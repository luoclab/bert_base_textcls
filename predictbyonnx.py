from predict.dataloader import  MyDataset
import torch
import onnxruntime
import os
...
test = MyDataset(sentences_tests_, labels=None, with_labels=False)
...
ort_session = onnxruntime.InferenceSession("./model_textcls1.onnx")
# ort_inputs = {'input': ([x[0],x[1],x[2]])}
# ort_inputs = {'input.1':x[0].detach().numpy(),'attention_mask':([x[0].detach().numpy(),x[1].detach().numpy(),x[2].detach().numpy()]),'input.3':x[2].detach().numpy()}
ort_inputs = ([x[0],x[1],x[2]])
